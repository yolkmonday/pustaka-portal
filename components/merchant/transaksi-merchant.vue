    ...
    <div class="grid-4">
      ...
      <div class="flex gap-2">
        <button
          ...
          @click.prevent="page =1 ,transaksi.getData({...payload, mode: 'filter'}) "
        ><i class="fas fa-search"></i> Cari</button>
        <button
          class="btn-secondary w-full"
          @click.prevent="page =1, payload.trx_id = '', payload.kode_channel = '', payload.reff_id = '', payload.vendor_id = '', payload.kategori_id = '', payload.status = '', transaksi.getData({...payload, mode: ''})"
      ...
      <table class="mt-2 w-max min-w-full">
        ...
        <tbody v-if="!transaksi.loading && transaksi.data">
          ...
          >
            ...
            <td class="p-4">
              <p
                :class="m.status ==='Failed' ? 'bg-red-100 text-red-500' : m.status === 'Success' ? 'bg-green-100 text-green-500' : m.status === 'Unpaid' ? 'bg-orange-100 text-orange-500' :  'bg-blue-100 text-blue-5000'"
                class="
                rounded-full
                px-3
                w-max
